...
namespace core::directx9hook {
    ...
    export void RegisterInitializeCallback(CallbackType callback) {
...
CleanAndReturn:
    ...
    */
    void PrepareMeasurement(IDirect3DDevice9* pDevice) {
        ...
        if (rs != D3D_OK) {
            ToFileMessage(TAG, rs, "pDevice->GetSwapChain failed");
            return;
        }
        ...
        pSwapChain->Release();
        if (rs != D3D_OK) {
            ToFileMessage(TAG, rs, "pSwapChain->GetPresentParameters failed");
            return;
        }
